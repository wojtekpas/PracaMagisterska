...
function showHideIDC(page)
{
	...
	else
	{
		...
		IDReplaceHtml($id('idc-showcomments-link'), "Pokaż komentarze");
		$id('idc-showcomments-link').href = 'javascript:showHideIDC();';
		$id('idc-showcomments-link').style.display = showCommentsStyle;
		IDReplaceHtml($id('idc-postcomment-link'), "Pokaż komentarze");
...
function showIDC(page)
{
	...
	showCommentsStyle = $id('idc-showcomments-link').style.display;
	postCommentStyle = $id('idc-postcomment-link').style.display;
	IDReplaceHtml($id('idc-showcomments-link'), "Ukryj komentarze");
	$id('idc-showcomments-link').href = 'javascript:showHideIDC();';
	$id('idc-showcomments-link').style.display = showCommentsStyle;
	IDReplaceHtml($id('idc-postcomment-link'), "Ukryj komentarze");
	$id('idc-postcomment-link').href = 'javascript:showHideIDC();';
	$id('idc-postcomment-link').style.display = postCommentStyle;
	if($id('idc-footer'))
...
function voteComment(commentid, vote) {
	...
	{
		showMsgBox("Przykro nam", "<p>Już oddałeś głos na ten komentarz.</p>", 0, listObj);
		return;
	}

	if(commentObj.comments[commentid] && (commentObj.comments[commentid].status==2 || commentObj.comments[commentid].status==6))
	{
		showMsgBox("Przykro nam", "<p>Nie można głosować na usunięte komentarze.</p>", 0, listObj);
		return;
	}
	...
	{
		showMsgBox("Przykro nam", "<p>Już oddałeś głos na ten komentarz.</p>", 0, $id("IDComment"+commentid));
		return;
	}
...
{
	IDReplaceHtml($id("adminOptions"), 'Wczytywanie...');
	$id("adminOptions").href = "javascript:void(0);";

	if(!commentObj.curUser.isLoggedIn)
	{
		showMsgBox("Przykro nam", "<p>Musisz być zalogowany, żeby zmieniać ustawienia.</p>", 0, listObj);
		return;
	}
	//changeDisabledLink();
...
function postComment(src)
{
	...
	if(src==0) //new thread
	{
		if( commentObj.postUsingTwitter ) {
			...
			{
				showMsgBox("Przykro nam", "<p>Twój komentarz jest trochę za długi. Spróbuj podzielić go na kilka krótszych.");
				return;
			}

			IDReplaceHtml($id("IDNewThreadSubmitLI"), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Wysyłanie..." />');
		...
		//Check for defaults
		if($id('IDCommentNewThreadText').value=='Tutaj wpisz tekst!')
		{
			showMsgBox("Przykro nam", "<p>Potrzebujemy żebyś przed wysłaniem napisał komentarz.</p>", 0);
			return;
		}

		if($id('txtEmailNewThread').value == '' &&  commentObj.newthreadType == 3)
		{
			showMsgBox("Przykro nam", "<p>W celu założenia konta podaj ważny adres email.</p>", 0);
			return;
		}

		if($id('txtNameNewThread').value == '' && commentObj.newthreadType == 0)
		{
			showMsgBox("Przykro nam", "<p>Podaj swoje imię i spróbuj wysłać komentarz ponownie</p>", 0);
			return;
		}

		/*if($id('txtNameNewThread').value.indexOf('&')>0)
		{
			showMsgBox("Przykro nam", "<p>Twoje imie nie może zawierać znaku &.</p>", 0);
			return;
		}*/
	...
	else
	{
		if( commentObj.postUsingTwitter ) {
			...
			IDReplaceHtml($id("IDReplyDivSubmitLI"), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Wysyłanie..." />');
			var requestObj = new buildRequestObj(theStr, 0, null, connectionErr);
			xs.make_request(requestObj);
		...
		//Check for defaults
		if($id('txtComment').value=='Tutaj wpisz tekst!')
		{
			showMsgBox("Przykro nam", "<p>Potrzebujemy żebyś przed wysłaniem napisał komentarz.</p>", 0);
			return;
		}
...
{
	IDReplaceHtml($id("IDReplyDivSubmitLI"), '<a class="idc-btn_l-secondary" href="javascript: hideReply();"><span></span><span class="idc-r">Anuluj</span></a><a class="idc-btn_l" href="javascript: postComment(1);"><span></span><span class="idc-r"><strong>Wyślij Komentarz</strong></span></a>');
	IDReplaceHtml($id("IDNewThreadSubmitLI"), '<a class="idc-btn_l" href="javascript: postComment(0);"><span></span><span class="idc-r"><strong>Wyślij Komentarz</strong></span></a>');
	showMsgBox("Przykro nam","<p>Wystąpił błąd komunikacji. Upłynął limit czasu połączenia.</p>", 2);
};
...
function showReply(commentid) {
	...
	if( commentObj.parentid )
		IDReplaceHtml($id("IDCommentPostReplyLink" + commentObj.parentid), "<a class='idc-btn_s' href='javascript: showReply(" + commentObj.parentid + ")'><span><input type='hidden' id='reqUsersOn' value='no' /></span><span class='idc-r'>Odpowiedz</span></a>");

	IDReplaceHtml($id("IDCommentPostReplyLink"+commentid), "<span class='idc-btn_s'><span></span><span class='idc-r'>Publikowanie...</span></span>");


	createCookie("IDReplyCommentId", commentid, 1);

	if($id("IDCommentReplyName") && commentObj.comments[commentid].displayName)
		IDReplaceHtml($id("IDCommentReplyName"), "Odpowiadanie użytkownikowi #user# ".replace(/#user#/, commentObj.comments[commentid].displayName));
...
function showMsgBox(header, text, type, objTop, otherButton)
{
	...
	p.className="idc-bottom";
		...
		IDReplaceHtml(link2, "<span></span><span class='idc-r'>Zamknij wiadomość</span>");
...
function showReportBox(commentId)
{
	...
	var h6 = $newEl("h6");
	var type = 0;
	var header = 'Zgłoś ten komentarz jako naruszający zasady';
	IDReplaceHtml(h6, header);
	var pText = $newEl('div');
	var text = '<p>'+$id('IDCustomReportTxt').innerHTML+'</p><p>Powód zgłoszenia:</p><textarea id="IDCCommentAdditional" class="idc-text_noresize" value="" style="width:90%;"></textarea><p><a href="javascript: reportThisComment('+commentId+');" class="idc-btn_s"><span></span><span class="idc-r">Zgłoś to!</span></a></p>';
...
function showReputationWhy() {
	showMsgBox("Wskaźnik reputacji IntenseDebate", "<p>Wskaźnik reputacji to miara jakości wcześniejszych komentarzy konkretnego użytkownika w ocenie innych użytkowników systemu. Jest jednym ze sposobów, dzięki którym wiadomo, że komentarz napisał ktoś o dobrej reputacji.</p> <img src='"+IDHost+"/images/id-rep_scale.png' alt='Skala reputacji' />", 0 );
};
...
{
	showMsgBox("Masz feedback? To świetnie!", "<form><label for='txtFeedbackEmail'>Email:</label><input type='text' id='txtFeedbackEmail' name='txtEmail' class='idc-text' onfocus=\"txtOnFocus(this, '');\" onblur=\"txtOnBlur(this, '');\" /><textarea name='txtFeedback' class='idc-text' id='txtFeedback' value='Feedback' onfocus=\"txtOnFocus(this, 'Feedback');\" onblur=\"txtOnBlur(this, 'Feedback');\"></textarea><a href='javascript: sendFeedback()' class='idc-btn_s'><span></span><span class='idc-r'>Wyślij opinię</span></a></form>", 0);
...
{
	showMsgBox("Po co się zapisać?", "<p><ul><li>Bądź bliżej ludzi, którzy komentują na twoich ulubionych blogach</li><li>Promuj siebie publikując linki do twoich kont na facebooku, twitterze, i innych.</li><li>Zdobądź czytelników twojego bloga lub strony</li><li>Pozwól znajomym śledzić twoje komentarze na wszystkich blogach używających identyfikacji komentarzy.</li></ul></p>", 0);
};
...
function collapseThread(commentid) {
	if($id("IDCommentCollapseLink" + commentid) && $id("IDCommentCollapseLink" + commentid).className == "idc-collapselink") {
		...
		if( $id("IDCommentSubThread" + commentid).childNodes.length == 0 ) { //content not loaded yet
			...
			IDReplaceHtml($id("IDCommentSubThread" + commentid), "<div class='idc-thread_loading'><img src='http://s.intensedebate.com/images/ajax-loader.gif' alt='Wczytywanie...' /> Wczytywanie...</div>");
...
function IDeditComment(commentid)
{
	...
	{
		showMsgBox("Przykro nam", "<p>Żeby edytować komentarz musisz być zalogowany.</p>", 0, null);
		return;
	}

	if(!commentObj.comments[commentid] || !commentObj.comments[commentid].commentid)
	{
		showMsgBox("Przykro nam", "<p>Ten komentarz nie istnieje...</p>", 0, null);
		return;
	}
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<a href="javascript: IDcancelEditComment('+commentid+');" class="idc-btn_l-secondary"><span></span><span class="idc-r">Anuluj</span></a><a href="javascript: IDsaveComment('+commentid+');" class="idc-btn_l"><span></span><span class="idc-r"><strong>Zapisz</strong></span></a>');
...
function IDsaveComment(commentid)
{
	if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
	{
		showMsgBox("Przykro nam", "<p>Żeby usunąć komentarz musisz być zalogowany.</p>", 0, null);
		return;
	}
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Wysyłanie..." />');
...
{
	var answer = confirm("Jesteś pewien, że chcesz usunąć ten komentarz?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
		...
		if(type<0 || type>1)
		{
			showMsgBox("Przykro nam", "<p>Ten zapis jest niepoprawny.</p>", 0, null);
			return;
		}
		...
		IDReplaceHtml($id("IDCommentPostReplyLinkDelete"+commentid), 'Usuwanie...');
...
{
	var answer = confirm("Jesteś pewien, że chcesz usunąć ten trackback?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
			showMsgBox("Przykro nam", "<p>Żeby usunąć trackback musisz być zalogowany.</p>", 0, null);
			return;
		}
		...
		{
			IDReplaceHtml($id("IDTrackBackDeleteLink"+trackbackid), 'Usuwanie...');
		...
		{
			showMsgBox("Przykro nam", "<p>Ten trackback nie istnieje...</p>", 0, null);
			return;
		}
	}
}
...
{
	var answer = confirm("Jesteś pewien, że chcesz dodać ten adres IP do czarnej listy?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
			showMsgBox("Przykro nam", "<p>Żeby dodać adres IP do czarnej listy musisz być zalogowany.</p>", 0, null);
			return;
		}
		...
		IDReplaceHtml($id("IDCommentPostReplyLinkBan"+commentid), 'Blokowanie...');
...
function IDCSubscribeByEmail(type)
{
	...
	IDReplaceHtml($id('IDCSubscribeSubmit'+type), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Wysyłanie..." />');
...
IDC.user_menu = {
    ...
    "add_friend": function ( uid ) {
            if ( commentObj && commentObj.curUser && !commentObj.curUser.isLoggedIn ) {
                    showMsgBox("Przykro nam", "<p>Musisz być zalogowany, żeby dodawać przyjaciół.</p>", 0, null);
                    return;
            }
