...
\begin{figure}[h]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Czasy działania dla rzadkich wielomianów nieparzystego stopnia dla PolynomialVector}
\end{figure}
...
\begin{table}
	\begin{tabular}{ |p{3.5cm}|p{3cm}|p{3cm}|p{3.5cm}|} 
		...
		Testowany wielomian & Czas dla PolynomialMap [s] & Czas dla PolynomialVector [s] & Współczynnik czasów \\
		\hline
		$W(x) = x^{101} + 1$ & 0.003 & 0.003 & 1 \\
		$W(x) = x^{201} + 1$ & 0.007 & 0.008 & 1.143 \\
		$W(x) = x^{401} + 1$ & 0.032 & 0.032 & 1 \\
		$W(x) = x^{801} + 1$ & 0.229 & 0.228 & 0.996 \\
		$W(x) = x^{1601} + 1$ & 2.404 & 2.2391 & 0.995 \\
		$W(x) = x^{3201} + 1$ & 29.822 & 29.725 & 0.997 \\
		\hline
	\end{tabular}
\end{table}
