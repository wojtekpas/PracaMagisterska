...
\begin{document}
	...
	\begin{itemize}
		...
		\item $W(x)=(x-10^{20}-0.1)(x-10^{30}-0.1)(x-10^{40}-0.01)$
	\end{itemize}
	
	Moja aplikacja skutecznie porawdziła sobie z każdym przypadkiem. Z kolei drugi program, zwrócił następujące wyniki.
	\begin{itemize}
		\item $\{10^{20}\}$
	...
	Następnymi trzema kategoriami jest testowanie niewielkich pierwiastków rzeczywistych, występujących w wielomianach stopni: czwartego, szóstego i ósmego. Kategorię te są analogicznę. Jedyną róznicą jest to, że ułamkowa część liczby, jest coraz bardziej dokładna.
